...
class ADC128D818:
    ...
    def _convert_raw_to_volts(self, raw: int, index: int) -> float:
        ...
        volts = value_12b * self._config.extResistorMultipliers[index] * ADC_LSB_V
        return volts
    ...
    def read_channel(self, index: int) -> Tuple[float, int]:
        ...
        if self._config.continuous is False:
        ...
        self._config.log.debug(
            f"Reading channel {index}: {volts:9.4f} (raw 0x{raw:04X})"
        )
        return volts, raw
